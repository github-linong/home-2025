...
/**
 * Verify the caller via api2 `/api/me`. In dev (DEV_SKIP_AUTH) we accept a
 * `devUserId` from the query string or a `wander_dev_uid` cookie so the game
 * is playable locally without a real login.
...
export async function verifyWithApi2(cookieHeader, opts = {}) {
  if (config.devSkipAuth) {
    ...
    const fromCookie = parseCookie(cookieHeader, "wander_dev_uid");
    const devUserId =
