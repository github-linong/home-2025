...
description: "服务端权威 2–9 人桌联机 Hold'em：房间、机器人、多手连续、协议契约、审计落盘与上线后踩坑修复。"
pubDate: "Jul 16 2026"
...
> 本文原为六人联机德州扑克首版工程计划存档；当晚已落地并上线。下文保留设计正文，并在文首补充**实现进度与纠偏**，避免读者按过期假设联调。
>
> 体验入口：[德州扑克](/texas-holdem/) · 代码：`apps/poker-realtime` + `apps/web` · 部署：`npm run deploy:poker-realtime` / `deploy:web`

# 联机德州扑克实施计划与落地纪要

## 实现进度（2026-07-16 晚间纠偏）

首版目标已从「纯计划」推进到**可玩的生产环境**。相对初稿，产品与工程上有这些明确变更：

| 项 | 初稿假设 | 实际落地 |
| --- | --- | --- |
| 人数 | 固定 6 人 | **2–9 人**（`maxSeats=9`，布局随入座人数切换） |
| 机器人 | 未写 | 房主可 `room.addBot` / `room.removeBot`；对局中也可加人，**下一手入局** |
| 进房 UX | 大厅文案等待开局 | **进房即进牌桌**，座位/准备态可见；工具栏可准备、开始、加减机器人、离开 |
| 分享链接 | 无 | 进房后 URL 写 `?room=房间码`，打开链接可自动加入 |
| 多手连续 | 设计有，易踩坑 | 一手结束约 **1.5s** 自动下一手；`matchVersion` **跨手单调递增**（不可每手从 1 重置） |
| 鉴权 | 每次操作可查 session | 浏览器 `/api/me` **5 分钟缓存 + 单飞**；WS 侧按 TTL/行动次数复验 |
| 握手竞态 | 未强调 | 鉴权完成前缓冲消息，下发 `session.ready` 后再允许客户端发 `room.create` |
| 文件形态 | 文中多为 `.ts` | 实现为 **`.js` / `.mjs`**（`hand-engine.js`、`runtime.js`、`gateway.js` 等） |

**已交付能力（对照原 Phase）**

- Phase 1–2：登录建房/入房、服务端权威发牌行动结算、边池、幂等、机器人填充 — **已上线**
- Phase 3：断线宽限、超时自动行动、多手连续、`stateVersion` 冲突与 `sync.request` — **已上线**（跨手版本曾导致 UI「本手已结束」卡住，已修）
- Phase 4：journal 落盘、replay、脱敏日志、限流 — **已具备**
- Phase 5：Astro 代理、脚本、nginx `/ws/poker`、部署脚本 — **已具备**

**仍不做（与初稿一致）**：观战、聊天、排行榜、充值/rebuy、Redis 多实例、commit-reveal 可验证公平。

### 关键踩坑（务必写进联调清单）

1. **跨手 `matchVersion`**：若每手从 `1` 重计，客户端水位已在上手升高，会把新手快照当旧包丢弃 → 桌面停在「本手已结束」。正确做法：同 match 内版本单调递增；客户端在 `handId` 变化 / `handStarted` 时重置本地水位作兜底。
2. **进房却看不到人**：若 UI 等到 `inMatch` 才 `showGame()`，大厅只显示房间码。应在 `room.create/join` 成功后即渲染牌桌座位。
3. **`/api/me` 刷屏**：不要在每次准备/加减机器人时强制拉 session；缓存即可。
4. **WS 鉴权竞态**：`onopen` 立刻 `room.create` 可能早于服务端挂上消息监听；必须等 `session.ready`。
...
- Phase 1：房间与连接 — **完成**
- Phase 2：服务端权威牌局 — **完成**
- Phase 3：重连、超时、多手、`stateVersion` — **完成**（含跨手版本纠偏）
- Phase 4：审计与限流 — **完成**
- Phase 5：联调与上线 — **完成**
...
4. **`handId` 变化或 `eventType=handStarted`**：先将本地水位清零再比较，避免跨手误杀。
...
| 前端体验 | 已上线 | 进房即牌桌、`?room=`、鉴权缓存、跨手版本兜底 |
| 运维     | 已具备 | health、部署脚本；发布回滚仍靠手动 |

**纠偏已写入文首**；后续改动优先更新「实现进度」表，再回写本节。
