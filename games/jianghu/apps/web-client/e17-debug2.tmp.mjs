...
const main = async () => {
  ...
  let server = null;
  if (!existing) { server = startServer(); await server.ready; console.log("server up"); }
  ...
  try {
    ...
    console.log("-- OVERWORLD rAF heartbeat --");
    for (let i = 0; i < 5; i++) {
    ...
    console.log("-- DUNGEON rAF heartbeat (both players) --");
    for (let i = 0; i < 8; i++) {
      const s = await pA.evaluate(() => ({
        vis: document.visibilityState,
    ...
    console.log("errA=" + JSON.stringify(errA.slice(0, 8)));
    ...
    console.log("rAF frames in 500ms = " + rafProbe);
