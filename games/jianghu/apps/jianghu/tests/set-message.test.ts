 ...
 *     对应主题 BOSS 宝箱→本主题套装（石牢→铁骨/荒冢→鬼影/熔窟→烈阳）；biome0 → 无套装（golden 不变）。
 */
...
// ------------------------------------------------------------------
// ④ BOSS 宝箱→背包 setId 映射（三主题闭环：石牢→铁骨、荒冢→鬼影、熔窟→烈阳、biome0→无）
// ------------------------------------------------------------------

test("BOSS 宝箱→背包 setId：三主题闭环（石牢→铁骨、荒冢→鬼影、熔窟→烈阳、biome0→无）", async () => {
  const store = new MemoryCharacterStore();
  const svc = new CharacterService({ store });
  ...
  await applyChestOpenToInventory(svc, "u-set-chest3", mkWorld(BIOME_MOLTEN_CAVERN), [{ itemId: 203, rarity: 3, affixes: [1, 2, 3, 4, 5] }], 2);
  ...
  const s3 = await svc.loadOrCreate("u-set-chest3");
  const s0 = await svc.loadOrCreate("u-set-chest0");
  assert.equal(s1.snapshot.inventory.items[0].setId, SET_IRONBONE, "石牢 BOSS 宝箱 → 铁骨");
  assert.equal(s2.snapshot.inventory.items[0].setId, SET_WRAITH, "荒冢 BOSS 宝箱 → 鬼影");
  assert.equal(s3.snapshot.inventory.items[0].setId, SET_BLAZING_SUN, "熔窟 BOSS 宝箱 → 烈阳");
