...
// ─────────────────────────────────────────────────────────────
// E22：装备分解（disassemble；C7 单一来源）
// ─────────────────────────────────────────────────────────────

/**
 * 分解产出强化石数按稀有度（索引 0=白/1=蓝/2=金/3=暗金，与 InventoryItem.rarity 对齐）。
 * 白装 0 石（垃圾保底药水）、蓝 1、金 2、暗金 3 —— 背包满（INVENTORY_CAP=60）后清理垃圾装备
 * 的回收通道，换强化石（E19 材料）+ 药水（E21），形成背包管理闭环。
 * **纯背包控制面操作**：不涉掉落 Rng 流 / 世界实体 → 不污染 playtest golden（D9）。
 */
export const DISASSEMBLE_STONES_BY_RARITY: Readonly<Record<number, number>> = {
  0: 0, // 白
  1: 1, // 蓝
  2: 2, // 金
  3: 3, // 暗金
...
/**
 * 分解固定产出药水数（×1 保底；主理人拍板：白装也给 1 瓶，垃圾分解也有用）。
 * 与强化石同次落库（Character.potions 累加）；分解是纯背包操作，不进 world 事件流。
 */
export const DISASSEMBLE_POTIONS = 1;
