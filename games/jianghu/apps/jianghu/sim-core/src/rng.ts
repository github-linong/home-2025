/**
 * rng.ts — 确定性 RNG（E1 / ADR-JH-ENG-03 D9）
 * ===========================================================================
 * 提供 splitmix64 与 Xoshiro256+ 两种 64-bit 确定性算法。
 * 全部以 BigInt 运算 + 显式 & 0xFFFFFFFFFFFFFFFFn 掩码，保证跨语言
 * （TS 权威实现 ↔ 未来 GDScript/C# 端口，C7 golden）逐位一致。
 *
 * 复用参照：dungeon-online packages/sim-core/src/rng.ts（同源 D9 协议，直接搬用，
 *   不跨仓 import，保持 jianghu sim-core 自包含）。
 *
 * 设计要点：
 * - 纯函数式 advance：splitmix64Next / xoshiro256Next 返回「新值 + 新状态」，不依赖全局可变状态。
 * - 右移一律走 ushr（无符号右移），规避 BigInt 符号扩展，保证 64-bit 无符号语义。
 * - Rng 类为便捷封装（内部持有状态，非全局），供后续 dungeonGen / loot / spawning 按 seed 取流。
 *
...
/** 规整 seed 为 uint64 状态。接受 bigint / number / 数字字符串；任意字符串经 FNV-1a 哈希。 */
...
// ---------------- 字符串 → uint64 哈希（供 seed 字符串接入） ----------------

/**
 * FNV-1a 64-bit 字符串哈希（确定性，跨语言可复刻）。
 * 用于把人类可读的 seed 字符串规整为 uint64，喂给 splitmix64Seed → Rng。
 */
 ...
 * 供 dungeonGen / loot / spawning / 占位 world 按 seed 取流；
 * 同 seed 实例产生完全一致的序列（D9 确定性核心）。
 */
export class Rng {
